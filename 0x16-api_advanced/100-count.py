...
def count_words(subreddit, word_list):
    ...
    ocurrences = list(map(lambda word: words(word, result), word_list))
    print(ocurrences)


def words(word, result):
    """
    words
    """
    lowerlist = list(map(lambda title: title.lower(), result))
    count = lowerlist.count(word.lower())
    if count:
